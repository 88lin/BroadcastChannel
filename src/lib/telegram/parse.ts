...
function renderPostContent(
  ...
  const { channel, staticProxy, index = 0, id, title, isMultiChannel, allChannels } = options

  return [
    getForwardedFrom($, message),
    getReply($, message, { channel, isMultiChannel, channels: allChannels }),
...
export async function extractPost($: CheerioAPI, item: AnyNode | null, options: ExtractPostOptions): Promise<Post> {
  const { channel, telegramHost, staticProxy, index = 0, reactionsEnabled, isMultiChannel, isPrimaryChannel = true } = options
  ...
  const contentHtml = renderPostContent($, message, content, {
    ...
    reactionsEnabled,
    isMultiChannel,
    allChannels: options.allChannels,
    id,
    title,
  })
  const hasFile = message.find('.tgme_widget_message_document_wrap').length > 0
  const hasImage = message.find('.tgme_widget_message_photo_wrap').length > 0
    || message.find('.tgme_widget_message_video_wrap').length > 0
    || message.find('.tgme_widget_message_roundvideo_wrap').length > 0
    || message.find('.link_preview_image').length > 0
